(function () {
    var API_BASE = "/api/motas";
    var STORAGE_KEY = "motas.adminApiKey";
    ...
    function bindDeleteActions() {
        var buttons = document.querySelectorAll(".js-delete-mota");
        if (!buttons.length) {
            return;
        }
        ...
        Array.prototype.forEach.call(buttons, function (button) {
            button.addEventListener("click", function () {
                ...
                var modelo = button.getAttribute("data-modelo") || "esta mota";
                ...
                fetch(API_BASE + "/" + id, {
                    ...
                    .then(function (response) {
                        ...
                        window.location.href = "/motas?status=deleted";
                    })
    ...
    function exportCompressed(canvas, quality, maxBytes, resolve, reject) {
        canvas.toBlob(function (blob) {
            ...
            if (blob.size <= maxBytes || quality <= 0.45) {
                resolve(new File([blob], "mota.jpg", { type: "image/jpeg" }));
                return;
            }
    ...
    function redirectFromQrValue(value) {
        ...
        window.location.href = "/motas/codigo/" + encodeURIComponent(extractCodigo(value));
    }
    ...
    function extractCodigo(value) {
        ...
        var match = trimmed.match(/\/motas\/codigo\/([^/?#]+)/i);
        if (match && match[1]) {
    ...
    function showStatusFromQuery() {
        ...
        if (status === "created") {
            showFeedback("Mota criada com sucesso.", "success");
        } else if (status === "updated") {
            showFeedback("Mota atualizada com sucesso.", "success");
        } else if (status === "deleted") {
            showFeedback("Mota apagada com sucesso.", "success");
        }
    }
